...
class DocumentPipeline:
    ...
    async def process_document(
        ...
        enable_mm = enable_multimodal if enable_multimodal is not None else self.config.enable_multimodal
        ...
        logger.info(f"Parsed document with {len(parse_result.content_list)} content blocks")
        ...
        if enable_mm and multimodal_blocks:
            ...
            semaphore = asyncio.Semaphore(max(self.config.multimodal_max_concurrency, 1))
            ...
            tasks = [asyncio.create_task(_process_block(idx, block)) for idx, block in multimodal_blocks]
    ...
    def _create_combined_chunks(
        ...
        for i, text in enumerate(result.text_chunks):
            chunk_id = compute_mdhash_id(f"{result.file_name}:text:{i}", prefix="chunk-")
            chunks.append({
                "id": chunk_id,
                "content": text,
                "is_multimodal": False,
        ...
        for i, mm_item in enumerate(result.multimodal_items):
            chunk_id = compute_mdhash_id(
                f"{result.file_name}:{mm_item.content_type}:{i}",
                prefix="chunk-"
            )
